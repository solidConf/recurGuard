...
void push(unsigned char** oldArr, unsigned char* newVal, unsigned int* instrumentNum){
    *(oldArr + (*instrumentNum)) = newVal;
    (*instrumentNum)++;
}
void pop(unsigned char** oldArr, unsigned int* instrumentNum){
    if((*instrumentNum) < 1) return;
    (*instrumentNum)--;
    free(*(oldArr + (*instrumentNum)));
}


void _entryCheck(unsigned char** oldArr, unsigned char* newVal, unsigned int* times, unsigned int* instrumentNum){
//    printf("check the instruments %s\n", newVal);
    (*times)++;
//    printf("times:%d\n", *times);
    if (!oldArr){
...
//            *(oldArr + (*instrumentNum)) = newVal;
//            (*instrumentNum)++;
            push(oldArr, newVal, instrumentNum);
        }
...
void _checkReturn(unsigned char** oldArr, unsigned char* newVal, unsigned int* times, unsigned int* instrumentNum){
    if ((*instrumentNum) < 1000){
//        printf("the instrument Number%d\n", (*instrumentNum));
        if ((*times)<=1000&& (*times)%100==0){
//            *(oldArr + (*instrumentNum)) = newVal;
//            (*instrumentNum)++;
            pop(oldArr, instrumentNum);
        }
